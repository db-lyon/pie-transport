...
namespace UEMCPPIE
{
	...
	void FPIEInputRecorder::OnBeginPIE(bool /*bIsSimulating*/)
	{
		...
		if (Pending.bTakeRecord)
		{
			...
			UE_LOG(LogPIEStudio, Log, TEXT("[PIE-REC] take_record: %s (%s)"),
		...
		UE_LOG(LogPIEStudio, Log, TEXT("[PIE-REC] Armed → BeginPIE: id=%s, waiting for pawn"), *CurrentId);
	}
	...
	void FPIEInputRecorder::ApplyFPSPin(UWorld* PIEWorld)
	{
		...
		UE_LOG(LogPIEStudio, Log, TEXT("[PIE-REC] %s"), *Cmd);
	}
	...
	FRecorderFinishResult FPIEInputRecorder::FinaliseCurrent()
	{
		...
		if (ActorRows.Num() > 0)
		{
			...
			{
				UE_LOG(LogPIEStudio, Warning, TEXT("[PIE-REC] tracked.jsonl write failed: %s"), *WriteErr);
			}
			else
			{
		...
		UE_LOG(LogPIEStudio, Log, TEXT("[PIE-REC] Recorded %d frames (%.2fs) → %s"),
