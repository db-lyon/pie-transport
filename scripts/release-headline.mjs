 ...
 *
 * Invoke as `node scripts/release-headline.mjs` from CI.
 */
...
export function validate(items) {
  ...
  if (joined.length > MAX_JOINED_LEN) {
    throw new ValidationError(
      `Joined headline is ${joined.length} chars; max is ${MAX_JOINED_LEN}.`
    );
  }
  return joined;
}
...
function main() {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "pie-transport-relnotes-"));
